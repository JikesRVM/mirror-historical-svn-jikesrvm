...
/**
 * An instance of this class provides a mapping from symbolic register to a set
 * of restricted registers.
 * 
 * Each architcture will subclass this in a class RegisterRestrictions.
 */
...
// private final HashMap<Register, RestrictedRegisterSet> hash = new
// HashMap<Register, RestrictedRegisterSet>();
 ...
 * 
 * PRECONDITION: the instructions in each basic block are numbered in increasing
 * order before calling this. The number for each instruction is stored in its
 * <code>scratch</code> field.
 */
...
/*
 * public void iterateOverSet(){ Iterator<Interval> iKeys =
 * intervalHash.keySet().iterator(); Iterator<Register> rKeys =
 * hash.keySet().iterator(); Iterator<Register> noSpillRegisters =
 * noSpill.iterator(); Iterator<Interval> noSpillIntervals =
 * intervalNoSpill.iterator();
 * 
 * System.out.println("intervals and its restrcited register set");
 * while(iKeys.hasNext()){ Enumeration<Register> enume = phys.enumerateAll();
 * Interval i = iKeys.next(); RestrictedRegisterSet set = intervalHash.get(i);
 * System
 * .out.println("Basic Interval's reg "+i.getContainer().getRegister().toString
 * ()); while(enume.hasMoreElements()){ Register temp = enume.nextElement();
 * if(set.contains(temp)) temp.toString(); } }
 * 
 * System.out.println("Register and its restrcited register set");
 * while(rKeys.hasNext()){ Enumeration<Register> enume = phys.enumerateAll();
 * Register reg = rKeys.next();
 * 
 * System.out.println("CompoundInterval Interval's reg "+reg.toString());
 * RestrictedRegisterSet set = hash.get(reg); BitSet s =set.bitset;
 * while(enume.hasMoreElements()){ Register temp = enume.nextElement();
 * if(set.contains(temp)) temp.toString();
 * 
 * }
 ...
 * System.out.println("interval/ Register which cannot be spilled"); Interval i
 * = noSpillIntervals.next(); MappedBasicInterval m = (MappedBasicInterval)i;
 * System.out.println("Contains Registernum "+
 * m.container.getRegister().number);
 * 
 * }
 * 
 * while(noSpillRegisters.hasNext()){
 * System.out.println(" Register which cannot be spilled"); Register reg =
 * noSpillRegisters.next();
 * 
 * System.out.println("Contains Registernum "+ reg.number); } }
 */
/**
 * Record all the register restrictions dictated by live ranges on a particular
 * basic block.
 * 
 * PRECONDITION: the instructions in each basic block are numbered in increasing
 * order before calling this. The number for each instruction is stored in its
...
for (Enumeration<LiveIntervalElement> e = bb.enumerateLiveIntervals(); e
.hasMoreElements();) {
...
// remove this
VM._assert(i != null);
if (forceitfornow || i.getContainer().equals(i.getInterval())) {
...
// case.
for (InstructionEnumeration ie = bb.forwardInstrEnumerator(); ie
.hasMoreElements();) {
...
/**
 * Add architecture-specific register restrictions for a basic block. Override
 * as needed.
 * 
 * @param bb
 *        the basic block
 * @param symbolics
 *        the live intervals for symbolic registers on this block
 */
...
/**
 * Record that it is illegal to assign a symbolic register symb to any volatile
 * physical registers
 */
...
/**
 * Record that it is illegal to assign a symbolic register symb to any of a set
...
/**
 * Record that it is illegal to assign a symbolic register symb to a physical
 * register p
 */
 ...
 * 
 * @return true :yes, all volatiles are forbidden false :maybe, maybe not
 */
...
/**
 * Is it forbidden to assign symbolic register symb to physical register r in
 * instruction s?
 */
