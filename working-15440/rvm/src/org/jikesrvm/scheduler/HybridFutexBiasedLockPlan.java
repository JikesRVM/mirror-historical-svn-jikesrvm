 ...
 * @author Filip
 */
public class HybridFutexBiasedLockPlan extends AbstractThinLockPlan {
  public static HybridFutexBiasedLockPlan instance;
  ...
  public HybridFutexBiasedLockPlan() {
    instance=this;
  }
  ...
  public void boot() {
    if (false) {
      ...
      VM.sysWriteln("BL_STAT_THIN_WAIT   = ",BL_STAT_THIN_WAIT);
    }
  }
  ...
  public void unlock(Object o, Offset lockOffset) {
    ...
    for (int cnt=0;;cnt++) {
      ...
      if (stat.EQ(BL_STAT_BIASABLE)) {
        ...
        if (id.EQ(threadId)) {
          if (old.and(BL_LOCK_COUNT_MASK).isZero()) {
            Word changed=old.and(BL_UNLOCK_MASK).or(BL_STAT_THIN);
            if (Synchronization.tryCompareAndSwap(
                  o, lockOffset, old, changed)) {
              if (stat.EQ(BL_STAT_THIN_WAIT)) {
                FutexUtils.wake(o, lockOffset, 1);
              }
              return;
            }
          } else {
            Word changed=old.toAddress().minus(BL_LOCK_COUNT_UNIT).toWord();
            if (Synchronization.tryCompareAndSwap(
                  o, lockOffset, old, changed)) {
              return;
            }
      ...
      if (bits.and(BL_STAT_MASK).EQ(BL_STAT_BIASABLE)) {
        ...
        return
          ...
          !bits.and(BL_LOCK_COUNT_MASK).isZero();
                 ...
                 bits.and(BL_STAT_MASK).EQ(BL_STAT_THIN_WAIT)) {
  ...
  public final boolean attemptToMarkInflated(Object o, Offset lockOffset,
    ...
    if (oldLockWord.and(BL_STAT_MASK).EQ(BL_STAT_THIN) ||
      ...
      if (Synchronization.tryCompareAndSwap(
            o, lockOffset, oldLockWord, changed)) {
        if (oldLockWord.and(BL_STAT_MASK).EQ(BL_STAT_THIN_WAIT)) {
    ...
    for (;;) {
      ...
      if (old.and(BL_STAT_MASK).NE(BL_STAT_BIASABLE)) {
        if (VM.VerifyAssertions) VM._assert(old.and(BL_STAT_MASK).EQ(BL_STAT_THIN) ||
                                            ...
                                            old.and(BL_STAT_MASK).EQ(BL_STAT_THIN_WAIT));
        return false;
      } else {
        Word id = old.and(BL_THREAD_ID_MASK);
