 ...
 * @see HeavyCondLock
 * @see Lock */
public final class InterruptibleSpinLock implements Constants {
  /**
  ...
  public void lock() {
    ...
    i.awaitingInterruptibleSpinLock = this;
    if (p.awaitingInterruptibleSpinLock != this) { // make i first (and only) waiter on the contender chain
      i.contenderLink = i;
    ...
    } while (i.awaitingInterruptibleSpinLock == this);
  }
  ...
  public void unlock() {
    ...
    if (p != i) { // p is the last processor on the chain of processors contending for the lock
      ...
      if (p == q) { // only one processor waiting for the lock
        q.awaitingInterruptibleSpinLock = null; // q now owns the lock
