...
/**
 * A light-weigh condition variable and lock, like HeavyCondLock, but this
 ...
 */
@Uninterruptible
public final class LightCondLock {
  ThreadQueue waiting;
  ...
  public void lockNicely() {
    if (false) VM.sysWriteln("in lockNicely, this = ",Magic.objectAsAddress(this));
  ...
  private void waitImpl(long whenAwake) {
    ...
    while (waiting.isQueued(me) &&
      ...
      if (whenAwake==0) {
        ...
        me.monitor().timedWaitAbsoluteNicely(whenAwake);
      }
    }
    if (me.hasInterrupt) {
      throwInterrupt = true;
  ...
  @Unpreemptible
  public void lockedBroadcastNicely() {
    lockNicely();
    broadcast();
    unlock();
  }
