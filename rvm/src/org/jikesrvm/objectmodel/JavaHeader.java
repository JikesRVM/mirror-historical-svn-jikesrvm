...
public class JavaHeader implements JavaHeaderConstants {
  ...
  public static void fillInReplica(Object fromObj, Object toObj, int numBytes, boolean isArrayType) {
    if (VM.VerifyAssertions) {
      VM._assert(Sapphire.inFromSpace(ObjectReference.fromObject(fromObj)));
      VM._assert(Sapphire.inToSpace(ObjectReference.fromObject(toObj)));
      VM._assert(ForwardingWord.isForwarded(ObjectReference.fromObject(fromObj)));
      VM._assert(!ForwardingWord.isForwarded(ObjectReference.fromObject(toObj)));
      VM._assert(DYNAMIC_HASH_OFFSET && ADDRESS_BASED_HASHING);
    }

    int copyBytes = numBytes; // Default copy everything
    ...
    int bytesTakenUpByHeader;
    if (isArrayType)
      bytesTakenUpByHeader = ARRAY_HEADER_SIZE;
    else
      bytesTakenUpByHeader = SCALAR_HEADER_SIZE;

    copyBytes -= bytesTakenUpByHeader; // don't copy the header portion

    Address toAddress = Magic.objectAsAddress(toObj).minus(OBJECT_REF_OFFSET).plus(bytesTakenUpByHeader); // Low memory word of source object
    Address fromAddress = Magic.objectAsAddress(fromObj).minus(OBJECT_REF_OFFSET).plus(bytesTakenUpByHeader);
    
    if (Options.verbose.getValue() >= 10) {
      VM.sysWrite("Copying object "); VM.sysWriteln(ObjectReference.fromObject(fromObj));
      if (isArrayType)
        VM.sysWriteln("Is an array type");
      else
        VM.sysWriteln("is a scalar type");

      VM.sysWrite("Starting copying from "); VM.sysWriteln(fromAddress);
      VM.sysWrite("Copying to "); VM.sysWriteln(toAddress);
      VM.sysWrite("Copying length "); VM.sysWriteln(copyBytes);

      VM.sysWriteln("Initial state of blank replica ");
      for (int i = 0; i < numBytes; i += 4) {
        VM.sysWrite(Magic.objectAsAddress(fromObj).minus(OBJECT_REF_OFFSET).plus(i));
        VM.sysWrite("=");
        VM.sysWrite(Magic.objectAsAddress(fromObj).minus(OBJECT_REF_OFFSET).plus(i).loadAddress());
        VM.sysWrite(" ");
        VM.sysWrite(Magic.objectAsAddress(toObj).minus(OBJECT_REF_OFFSET).plus(i));
        VM.sysWrite("=");
        VM.sysWriteln(Magic.objectAsAddress(toObj).minus(OBJECT_REF_OFFSET).plus(i).loadAddress());
      }
    }

    Memory.aligned32Copy(toAddress, fromAddress, copyBytes); // Do the copy of all the slots
    ...
    if (Options.verbose.getValue() >= 10) {
      VM.sysWrite("Dumping replicated values for "); VM.sysWriteln(ObjectReference.fromObject(fromObj));
      for (int i=0; i<numBytes; i += 4) {
        VM.sysWrite(Magic.objectAsAddress(fromObj).minus(OBJECT_REF_OFFSET).plus(i));
        VM.sysWrite("=");
        VM.sysWrite(Magic.objectAsAddress(fromObj).minus(OBJECT_REF_OFFSET).plus(i).loadAddress());
      ...
      if (MemoryManagerConstants.MOVES_OBJECTS) {
        ...
        if (originalHashState.EQ(HASH_STATE_HASHED)) {
          ...
          if (!Sapphire.inFromSpace(ObjectReference.fromObject(o))) {
            ...
            ObjectReference forwarded = ForwardingWord.getReplicaPointer(obj);
            if (forwarded != null) {
              ...
              ForwardingWord.atomicMarkBusy(obj);
              forwarded = ForwardingWord.getReplicaPointer(obj);
              if (forwarded != null) {
  ...
  public static void writeReplicaPointer(ObjectReference o, ObjectReference ptr) {
