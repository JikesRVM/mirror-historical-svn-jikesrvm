...
public abstract class CompiledMethod implements SizeConstants {
  ...
  public final void setObsolete() {
    if (Selected.Constraints.get().replicatingGC() && Plan.gcInProgress()) setActiveOnStack(); // avoid a thread which has already
                                                                                               // been scanned running an obsolete
                                                                                               // method that will be GC'd shortly
    flags |= OBSOLETE;
  }
