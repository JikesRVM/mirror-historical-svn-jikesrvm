...
public abstract class VM_Thread {
  ...
  public final void terminate() {
    ...
    VM_Callbacks.notifyThreadEnd(this);
  ...
  /**
   * Get the internal state of the thread in a manner compatible with the JDWP 
   * specification. it differs a little from the interpretation in {@link 
   * #getState()}.
   * @return thread state
   * @see #getState()
   * @see java.lang.Thread#getState()
   * @see gnu.classpath.jdwp.VMVirtualMachine#getThreadStatus(Thread)
   */
  @Interruptible
  public final int getJdwpState(){
	  switch (state) {
	    case NEW:
	    case RUNNABLE:
	      return JdwpConstants.ThreadStatus.RUNNING;
	    case BLOCKED:
	      return JdwpConstants.ThreadStatus.MONITOR;
	    case WAITING:
	    case SUSPENDED:
	    case OSR_SUSPENDED:
	    case JOINING:
	    ...
	    case TERMINATED:
	      return JdwpConstants.ThreadStatus.ZOMBIE;
	    }
	    VM.sysFail("Unknown thread state " + state);
	    return -1;
  }
