...
public abstract class VM_Scheduler {
  ...
  public static FrameRecord[] dumpStacktoArray(Address ip, Address fp) {
	  ...
	  VM.sysWrite("-- Stack --\n");	  
	  final VM_LinkedList<FrameRecord> frames = new VM_LinkedList<FrameRecord>();
      VM.sysWriteln();
...
//						VM.sysWriteln("found instance", (null == instance)? "null": instance.getClass().getName());
	        ...
	        if (compiledMethod.getCompilerType() == VM_CompiledMethod.TRAP) {
	          ...
	          final int lineNumber = compiledMethod.findLineNumberForInstruction(instructionOffset);
