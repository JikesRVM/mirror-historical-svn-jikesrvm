 ...
 * thin state in which case it looks like 0T..TC..C where
 * T is the thread id of the thread that owns the lock and
...
interface VM_ThinLockConstants {
  ...
  static final int NUM_BITS_RC         = Math.max(2, (VM_JavaHeader.NUM_THIN_LOCK_BITS)/4);
  static final int NUM_BITS_TID        = VM_JavaHeader.NUM_THIN_LOCK_BITS - NUM_BITS_RC - 1;
  ...
  static final int TL_LOCK_ID_MASK     = (-1 >>> (32 - (NUM_BITS_RC + NUM_BITS_TID))) << TL_LOCK_ID_SHIFT;
  static final int TL_FAT_LOCK_MASK    = 1 << (VM_JavaHeader.THIN_LOCK_SHIFT + NUM_BITS_RC + NUM_BITS_TID);
