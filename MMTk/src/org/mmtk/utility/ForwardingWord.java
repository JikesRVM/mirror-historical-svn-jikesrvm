...
public class ForwardingWord {
  ...
  public static void setReplicaPointer(ObjectReference fromSpace, ObjectReference toSpace) {
    ...
    VM.objectModel.writeReplicaPointer(fromSpace, toSpace); // write FP into spare word
    if (VM.VERIFY_ASSERTIONS) {
      VM.assertions._assert(!isForwarded(fromSpace));
      VM.assertions._assert(isBusy(fromSpace));
      VM.assertions._assert(!isBusy(toSpace));
  ...
  @Inline
  // may be called to get either FP or BP
  public static ObjectReference getReplicaPointer(ObjectReference object) {
    ObjectReference forwarded = VM.objectModel.getReplicaPointer(object);
    return forwarded;
  }
