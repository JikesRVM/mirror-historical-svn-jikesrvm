...
@Uninterruptible public final class ObjectModel extends org.mmtk.vm.ObjectModel implements org.mmtk.utility.Constants,
  ...
  public ObjectReference fillInBlankDoubleRelica(ObjectReference from, Address toSpace, int bytes) {
    ...
    if (Options.verbose.getValue() >= 10) {
      Log.write("Dumping details of double alloc replica "); Log.writeln(toObj);
      for (int i = 0; i < bytes; i += 4) {
        Log.write(Magic.objectAsAddress(from).minus(16).plus(i)); Log.write("=");
        Log.write(Magic.objectAsAddress(from).minus(16).plus(i).loadAddress()); // hard coded hack
        Log.write(" ");
        Log.write(Magic.objectAsAddress(toObj).minus(16).plus(i)); Log.write("=");
        Log.writeln(Magic.objectAsAddress(toObj).minus(16).plus(i).loadAddress());  // hard coded hack
      }
      Log.write("Return address "); Log.writeln(toObj);
    }
    return toObj;
  }
  ...
  private ObjectReference createBlankReplicaScalar(ObjectReference from, TIB tib, RVMClass type, int allocator) {
    ...
    context.postCopy(from, to, ObjectReference.fromObject(tib), bytes, allocator);
    if (Options.verbose.getValue() >= 10) {
      Log.write("Dumping details of blank replica "); Log.writeln(to);
      for (int i = 0; i < bytes; i += 4) {
        Log.write(Magic.objectAsAddress(from).minus(16).plus(i)); Log.write("=");
        Log.write(Magic.objectAsAddress(from).minus(16).plus(i).loadAddress()); // hard coded hack
        Log.write(" ");
        Log.write(Magic.objectAsAddress(to).minus(16).plus(i)); Log.write("=");
        Log.writeln(Magic.objectAsAddress(to).minus(16).plus(i).loadAddress());  // hard coded hack
      }
    }
    return to;
  }
  ...
  public void fillInReplica(ObjectReference from, ObjectReference to) {
    if (VM.VERIFY_ASSERTIONS) {
      VM.assertions._assert(Sapphire.inFromSpace(from));
      VM.assertions._assert(Sapphire.inToSpace(to));
    ...
    org.jikesrvm.objectmodel.JavaHeader.fillInReplica(from.toObject(), to.toObject(), bytes, isArrayType);
  }
  ...
  public void writeReplicaPointer(ObjectReference o, ObjectReference ptr) {
    org.jikesrvm.objectmodel.JavaHeader.writeReplicaPointer(o, ptr);
  }
